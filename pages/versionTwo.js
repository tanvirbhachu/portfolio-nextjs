...
export default function Home() {
  ...
  return (
    <div id='main' className={`scroll-smooth w-screen overflow-x-hidden bg-white dark:bg-black dark:text-white ${theme ? 'dark' : ''}`}>
        ...
        <div className={`md:absolute z-50 bg-transparent dark:text-white top-0 left-0 px-5 py-5 pointer-events-none transition-all ease-in snap-none flex justify-between w-full`}>
            <a onClick={() => {setTheme(n => !n); return false}} href="#" className="p-2 pointer-events-initial hover:scale-105 animate__animated animate__slideInLeft animate__fast">
                {theme ? <FiSun fontSize={25} /> : <FiMoon fontSize={25} />}
            </a>
